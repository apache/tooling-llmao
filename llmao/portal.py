"""Minimal HTML for control-plane status.

No chat UI. A later phase may grow a key/budget admin console here.
"""
...
def render_index(settings: Settings, ident: Optional[Identity]) -> str:
    if ident is None:
        signin = '<a class="btn" href="/auth?login=/">Sign in with ASF</a>'
        who = ""
    else:
...
<main class="shell">
  <div class="console">
    ...
    point your tools at the LiteLLM proxy with a project PAT.</p>
    {who}
    {signin}
    <div class="modeline">litellm: {_esc(settings.litellm_mode)}</div>
  </div>
</main>
</body></html>"""
